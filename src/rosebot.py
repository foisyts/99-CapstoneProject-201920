"""
  ...
  Author:  Your professors (for the framework and lower-level code)
    and Tristen Foisy, Thomas Hoevener, Sam Hedrick.
...
class DriveSystem(object):
    ...
    def spin_clockwise_until_sees_object(self, speed, area):
        ...
        self.go(speed, -speed)
        while True:
            if int(self.sensor_system.camera.get_biggest_blob().height * self.sensor_system.camera.get_biggest_blob().width) >= area:
                self.stop()
...
class ArmAndClaw(object):
    ...
    def move_arm_to_position(self, desired_arm_position):
        ...
        """
        first_pos = self.motor.get_position()
        if first_pos > desired_arm_position:
            while True:
                self.motor.turn_on(-100)
                changing_pos = self.motor.get_position()
                if changing_pos <= desired_arm_position:
                    self.motor.turn_off()
                    break
        elif first_pos < desired_arm_position:
            while True:
                self.motor.turn_on(100)
                changing_pos = self.motor.get_position()
                if changing_pos >= desired_arm_position:
